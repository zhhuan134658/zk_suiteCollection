...
const FormField: ISwapFormField = {
  ...
  fieldDidMount() {
    ...
    const value = form.getFieldValue('unit');
    form.onFieldExtendValueChange('unit', value => {
      if (value === null) {
        this.setState({
          theirName: '',
        });
  ...
  asyncSetFieldProps() {
    ...
    const promise = asyncSetProps(this, allData, 'theirUnit');
    promise.then(res => {
  ...
  setFieldValues(value = '') {
    ...
    form.setFieldExtendValue('unit', value);
  },
  methods() {
    return {
  ...
  fieldRender() {
    ...
    const field = form.getFieldInstance('theirUnit');
    const label = form.getFieldProp('theirUnit', 'label');
    const required = form.getFieldProp('theirUnit', 'required');
    const {
      visible,
      treeData,
      newOptine,
    ...
    return !viewMode ? (
      <CustomField className={'TheirUnit_class'} showLabel={false}>
